...
use space_game_protocol::SERVER_PORT;
...
fn parse_address(value: &str) -> Result<SocketAddr, StartError> {
    value
        .parse()
        .map_err(|error| format!("invalid SPACE_GAME_SERVER_ADDRESS: {error}"))
}

#[cfg(feature = "dev")]
fn configured_public_address() -> Result<SocketAddr, StartError> {
    match std::env::var("SPACE_GAME_SERVER_ADDRESS") {
...
#[cfg(not(feature = "dev"))]
fn configured_public_address() -> Result<SocketAddr, StartError> {
    let address = std::env::var("SPACE_GAME_SERVER_ADDRESS").map_err(|error| {
        format!(
            "SPACE_GAME_SERVER_ADDRESS must be set to a reachable IPv4 address on port 5000: {error}"
        )
    })?;
...
fn validate_public_address(address: SocketAddr) -> Result<SocketAddr, StartError> {
    if address.is_ipv4() && !address.ip().is_unspecified() && address.port() == SERVER_PORT {
        ...
        Err("SPACE_GAME_SERVER_ADDRESS must be a reachable IPv4 address on port 5000".to_owned())
    }
}
